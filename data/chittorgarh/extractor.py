    ...
    def _extract_subscription_data(self, html_content: str) -> dict:
        ...
        for table in tables:
            ...
            if 'subscription' in table_text and 'category' in table_text:
                ...
                headers = [self._clean(h.get_text()) for h in rows[0].find_all(['th', 'td'])]
                ...
                for idx, header in enumerate(headers):
                    ...
                    elif 'shares' in header_lower and ('applied' in header_lower or 'bid' in header_lower):
                        col_indices['shares_applied'] = idx
                    elif 'amount' in header_lower and 'offered' in header_lower:
                        col_indices['amount_offered'] = idx
                    elif 'amount' in header_lower and ('applied' in header_lower or 'bid' in header_lower):
                ...
                for row_idx, tr in enumerate(rows[1:], start=1):
                    ...
                    category = cells[col_indices['category']].strip() if col_indices['category'] is not None else None

                    if not category or category.lower() in ['total', 'total subscription']:
                        continue

                    row_data = {'category': category}

                    if col_indices['subscription_times'] is not None and col_indices['subscription_times'] < len(cells):
                        row_data['subscription_times'] = cells[col_indices['subscription_times']]

                    if col_indices['shares_offered'] is not None and col_indices['shares_offered'] < len(cells):
                        row_data['shares_offered'] = cells[col_indices['shares_offered']]

                    if col_indices['shares_applied'] is not None and col_indices['shares_applied'] < len(cells):
                        row_data['shares_applied'] = cells[col_indices['shares_applied']]

                    if col_indices['amount_offered'] is not None and col_indices['amount_offered'] < len(cells):
                        row_data['amount_offered'] = cells[col_indices['amount_offered']]

                    if col_indices['amount_applied'] is not None and col_indices['amount_applied'] < len(cells):
                        row_data['amount_applied'] = cells[col_indices['amount_applied']]
        ...
        for table in tables:
            ...
            if 'allocation' in table_text and 'category' in table_text:
                ...
                for row_idx, tr in enumerate(rows[1:], start=1):
                    ...
                    if not category or category.lower() in ['total', 'total allocation']:
                        continue
                    ...
                    if col_indices['amount'] is not None and col_indices['amount'] < len(cells):
                        row_data['amount'] = cells[col_indices['amount']]

                    if col_indices['size_percentage'] is not None and col_indices['size_percentage'] < len(cells):
                        row_data['size_percentage'] = cells[col_indices['size_percentage']]
    ...
    def _extract_from_text(self, html_content: str) -> dict:
        ...
        for field, pattern in field_patterns.items():
            ...
            if match:
                ...
                value = re.sub(r'(?:Read|Financial|Information|Documents|Key|Highlights).*$', '', value).strip()
                data[field] = value
            else:
                data[field] = None
    ...
    def _extract_dates_from_text(self, html_content: str) -> dict:
        ...
        for date_field, pattern in date_patterns.items():
            ...
            if match:
                ...
                value = re.sub(r'(?:Initiation|Refund|Read|Documents|Financial).*$', '', value).strip()
